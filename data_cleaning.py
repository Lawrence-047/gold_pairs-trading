...
def _stitch_by_volume_stable_with_next(prod_df: pd.DataFrame,
    ...
    confirm_streak = 0
    prev_main_id = None
    cond_prev = False  # ↑ 新增：上一日是否满足“候选量≥ratio×当前量”
    ...
    for _, row in first.iterrows():
        ...
        if cur_id is None:
            ...
            if cand_id != cur_id:
                ...
                # —— 信号日：首次从不满足 -> 满足比例条件（候选≠当前）
                if cond_ratio and (not cond_prev):
                    signal_today = True
                # 维护上一日状态（仅在候选≠当前时才延续；否则重置）
        ...
        out_rows.append({
            ...
            f"{product}_signal_T": bool(signal_today),    # ↑ 新增：信号日（你的 T）
        })
